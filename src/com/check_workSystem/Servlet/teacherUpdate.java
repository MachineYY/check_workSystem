...
public class teacherUpdate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		      ...
		      String sql="update teacher set tea_name=?,tea_pwd=?,tea_sex=?,tea_tel=? where tea_no=?";
		      java.sql.PreparedStatement ps=conn.prepareStatement(sql);
	          ps.setString(1,tea_name);
		      ps.setString(2,tea_pwd);
		      ps.setString(3,tea_sex);
		      ps.setString(4,tea_tel);
		      ps.setNString(5,tea_no);
		      ps.executeUpdate();
		      ps.close();
		      conn.close();
		...
		//response.sendRedirect("mange_teacher");
