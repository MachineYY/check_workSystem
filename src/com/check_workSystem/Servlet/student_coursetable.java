...
/**
 * Servlet implementation class student_coursetable
 */
@WebServlet("/student_coursetable")
public class student_coursetable extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public student_coursetable() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		    ...
		    String sql="select stu_class from student where stu_no="+"'"+u_id+"'";
			ResultSet rs=stmt.executeQuery(sql);//查询学生表
			if(rs.next()) {
				if(rs.getObject("stu_class")!="") {
					//System.out.print(rs.getObject("stu_class"));
					String stu_class = (String)(rs.getObject("stu_class"));
					rs.close();
					String sql1="select * from s_course where stu_class="+"'"+stu_class+"'";
					ResultSet rs1=stmt.executeQuery(sql1);//查询学生课程表
					List list = convertList(rs1);//将result结果集转存在list中 因为result对象会关闭
					//System.out.print(list);
					rs1.close();
		    		conn.close();
		    		stmt.close();
//					while(rs1.next()){
//					        int id = rs1.getInt("number");
//					        String course_no = rs1.getString("course_no");
//					        String course_name = rs1.getString("course_name");
//					        String course_week = rs1.getString("course_week");
//					        String course_day = rs1.getString("course_day");
//					        String course_section = rs1.getString("course_section");
//					        rs1.close();
//					        System.out.println("编号:"+id+" 课程编号："+course_no+" 课程名："+course_name+" 课程周："+course_week+" 周几："+course_day+" 节次："+course_section);
//					    }
					//打印页面
